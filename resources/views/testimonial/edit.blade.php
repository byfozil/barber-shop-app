                        ...
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form class="forms-sample" action="{{ route('testimonial_edit_put') }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <input type="hidden" name="id"
                                            value="{{ old('id') ? old('id') : $testimonial->id }}">
                                        <div class="form-group">
                                            <label for="author">Author</label>
                                            <input type="text" class="form-control" id="author" name="author"
                                                placeholder="Author"
                                                value="{{ old('author') ? old('author') : $testimonial->author }}">
                                        </div>
                                        <div class="form-group">
                                            <label for="content">Content</label>
                                            <textarea class="form-control" id="content" name="content" placeholder="Content" rows="4">{{ old('content') ? old('content') : $testimonial->content }}</textarea>
                                        </div>
                                        @if ($errors->any())
                                            <div class="form-group">
                                        ...
                                        @endif
                                        <button type="submit" class="btn btn-primary me-2">Edit Testimonials</button>
                                        <a href="../../testimonials" class="btn btn-dark">Cancel</a>
                                    </form>
                                </div>
                            </div>
                        </div>
